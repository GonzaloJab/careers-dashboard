...
def _ensure_job(conn: sqlite3.Connection, job_id: Optional[int]) -> int:
    ...
    cur = conn.execute(
        """
        INSERT INTO jobs (public_id,title,team,location,remote,seniority,type,description,status,questions,criteria,ai_requirements,rejection_template,rejection_email_subject,contact_email_subject,contact_email_body)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            "local-test",
